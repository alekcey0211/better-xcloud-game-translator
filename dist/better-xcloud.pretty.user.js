...
var SUPPORTED_LANGUAGES = {
 ...
 "game-translator-mode": "Recognition mode",
 "game-translator-mode-all-text": "All screen text (experimental)",
 "game-translator-mode-note": "All text scans the full frame and may reduce performance on Quest.",
 "game-translator-mode-subtitles": "Subtitles only",
...
class GlobalSettingsStorage extends BaseSettingsStorage {
 static DEFINITIONS = {
  ...
  "gameTranslator.mode": {
   ...
   default: "subtitles",
   options: {
    subtitles: t("game-translator-mode-subtitles"),
    "all-text": t("game-translator-mode-all-text")
   },
...
var DETECTION_WIDTH = 640, MAX_OCR_WIDTH = 1280, SCENE_OCR_WIDTH = 960, FULL_FRAME_REGION = { x: 0, y: 0, width: 1, height: 1 }, OCR_REGIONS = {
...
class TranslatorFrameCapture {
 ...
 region;
 mode;
 constructor(region, mode = "subtitles") {
  this.region = OCR_REGIONS[region], this.mode = mode, this.$detectionCanvas.width = DETECTION_WIDTH, this.$detectionCanvas.height = 1, this.detectionContext = this.$detectionCanvas.getContext("2d", {
   alpha: !1,
 ...
 getRegion() {
  return this.mode === "all-text" ? FULL_FRAME_REGION : this.region;
 }
 getDisplayElement() {
 ...
 captureForDetection() {
  ...
  let region = this.getRegion(), targetHeight = Math.max(1, Math.round(DETECTION_WIDTH * ($video.videoHeight * region.height) / ($video.videoWidth * region.width)));
 ...
 captureForSceneOcr() {
  let $video = this.screenshotManager.getCurrentFrameSource("video");
  if (!($video instanceof HTMLVideoElement) || !$video.videoWidth || !$video.videoHeight) return null;
  let targetHeight = Math.max(1, Math.round(SCENE_OCR_WIDTH * $video.videoHeight / $video.videoWidth));
  if (this.$ocrSourceCanvas.width !== SCENE_OCR_WIDTH || this.$ocrSourceCanvas.height !== targetHeight) this.$ocrSourceCanvas.width = SCENE_OCR_WIDTH, this.$ocrSourceCanvas.height = targetHeight;
  if (this.$ocrCanvas.width !== SCENE_OCR_WIDTH || this.$ocrCanvas.height !== targetHeight) this.$ocrCanvas.width = SCENE_OCR_WIDTH, this.$ocrCanvas.height = targetHeight;
  if (!this.screenshotManager.captureFrame(this.$ocrSourceCanvas, {
   elementType: "video",
...
class TesseractOcrEngine {
 ...
 async getWorker() {
  ...
  if (!this.workerPromise) this.workerPromise = import_tesseract.createWorker("eng", import_tesseract.OEM.LSTM_ONLY, {
    ...
    }), this.pageSegmentationMode = import_tesseract.PSM.SINGLE_BLOCK, this.terminated)
 ...
 async recognize(image) {
  ...
  await this.setPageSegmentationMode(worker, import_tesseract.PSM.SINGLE_BLOCK);
 ...
 async recognizeScene(image) {
  let worker = await this.getWorker();
  await this.setPageSegmentationMode(worker, import_tesseract.PSM.SPARSE_TEXT);
  let result = await worker.recognize(image, {}, { text: !0, blocks: !0 }), lines = [];
  for (let block of result.data.blocks || [])
   for (let paragraph of block.paragraphs)
    for (let line of paragraph.lines) {
     let text = cleanRecognizedLine(line.text, line.confidence), width = line.bbox.x1 - line.bbox.x0, height = line.bbox.y1 - line.bbox.y0;
 ...
 async setPageSegmentationMode(worker, mode) {
  if (this.pageSegmentationMode === mode) return;
  await worker.setParameters({ tessedit_pageseg_mode: mode }), this.pageSegmentationMode = mode;
 }
 async terminate() {
  this.terminated = !0;
  try {
   ...
   this.worker = null, this.workerPromise = null, this.pageSegmentationMode = null;
  }
 }
}
...
function createSceneSignature(frame) {
 ...
 for (let row = 0;row < SIGNATURE_ROWS; row++) {
  let y0 = Math.floor(row * frame.height / SIGNATURE_ROWS), y1 = Math.max(y0 + 1, Math.floor((row + 1) * frame.height / SIGNATURE_ROWS));
  for (let column = 0;column < SIGNATURE_COLUMNS; column++) {
   let x0 = Math.floor(column * frame.width / SIGNATURE_COLUMNS), x1 = Math.max(x0 + 1, Math.floor((column + 1) * frame.width / SIGNATURE_COLUMNS)), stepX = Math.max(1, Math.floor((x1 - x0) / 4)), stepY = Math.max(1, Math.floor((y1 - y0) / 4)), total = 0, samples = 0, darkest = 255, brightest = 0;
   for (let y = y0;y < y1; y += stepY)
    for (let x = x0;x < x1; x += stepX) {
     let index = (y * frame.width + x) * 4, luminance = 54 * frame.data[index] + 183 * frame.data[index + 1] + 19 * frame.data[index + 2] >> 8;
     total += luminance, samples++, darkest = Math.min(darkest, luminance), brightest = Math.max(brightest, luminance);
    }
   let averageBucket = Math.floor(total / samples / 16), contrastBucket = Math.floor((brightest - darkest) / 16);
...
class TranslationOverlay {
 ...
 sceneItems = [];
 contentRect = null;
 sceneClearTimerId = null;
 constructor($video, settings) {
  this.settings = settings, this.$host = $video.closest("#game-stream") || $video.parentElement, this.$root = CE("div", { class: "bx-game-translator-overlay" }, this.$subtitle = CE("div", { class: "bx-game-translator-subtitle bx-gone" }, this.$translation = CE("div", { class: "bx-game-translator-translation" }), this.$original = CE("div", { class: "bx-game-translator-original" })), this.$scene = CE("div", { class: "bx-game-translator-scene bx-gone" }), this.$region = CE("div", { class: "bx-game-translator-region bx-gone" }), this.$debug = CE("div", { class: "bx-game-translator-debug bx-gone" })), this.$subtitle.setAttribute("aria-live", "polite"), this.$host.appendChild(this.$root), this.applySettings(settings);
 }
 applySettings(settings) {
  this.settings = settings, this.$subtitle.style.fontSize = `${settings.fontSize}px`, this.$subtitle.style.backgroundColor = `rgba(0, 0, 0, ${settings.backgroundOpacity / 100})`, this.$original.classList.toggle("bx-gone", !settings.showOriginal || !this.originalText), this.$region.classList.toggle("bx-gone", !settings.debugRegion), this.$debug.classList.toggle("bx-gone", !settings.debugRegion), this.renderSceneItems();
 }
 updateGeometry($player, region) {
  if (!$player || !$player.isConnected) return;
  let contentRect = getRenderedContentRect($player);
  if (!contentRect.width || !contentRect.height) return;
  this.contentRect = contentRect, this.$subtitle.style.left = `${contentRect.left + contentRect.width / 2}px`, this.$subtitle.style.bottom = `${window.innerHeight - contentRect.bottom + contentRect.height * this.settings.verticalPosition / 100}px`, this.$subtitle.style.maxWidth = `${contentRect.width * 0.9}px`, this.$region.style.left = `${contentRect.left + contentRect.width * region.x}px`, this.$region.style.top = `${contentRect.top + contentRect.height * region.y}px`, this.$region.style.width = `${contentRect.width * region.width}px`, this.$region.style.height = `${contentRect.height * region.height}px`, this.renderSceneItems();
 }
 show(originalText, translatedText) {
  this.clearSceneImmediately(), this.cancelClear(), this.originalText = originalText, this.$translation.textContent = translatedText, this.$original.textContent = originalText, this.$original.classList.toggle("bx-gone", !this.settings.showOriginal || !originalText), this.$subtitle.classList.toggle("bx-gone", !translatedText), this.visibleUntil = translatedText ? performance.now() + getTranslationDisplayDuration(translatedText, this.settings.minimumDisplayTime) : 0;
 }
 showError(message) {
  this.show("", message);
 }
 showScene(items) {
  this.cancelClear(), this.cancelSceneClear(), this.hide(), this.sceneItems = items, this.renderSceneItems(), this.$scene.classList.toggle("bx-gone", !items.length);
 }
 clearScene() {
  if (!this.sceneItems.length || this.sceneClearTimerId !== null) return;
  this.sceneClearTimerId = window.setTimeout(() => {
   this.sceneClearTimerId = null, this.clearSceneImmediately();
  }, this.settings.minimumDisplayTime);
 }
 resetContent() {
  this.cancelClear(), this.cancelSceneClear(), this.hide(), this.clearSceneImmediately();
 }
 clear() {
 ...
 destroy() {
  this.cancelClear(), this.cancelSceneClear(), this.$root.remove();
 }
 hide() {
 ...
 renderSceneItems() {
  ...
  if (!contentRect || !this.sceneItems.length) return;
  let elements = this.sceneItems.map((item2) => {
   let height = Math.max(18, item2.box.height * contentRect.height), $item = CE("div", { class: "bx-game-translator-scene-text" }, CE("div", { class: "bx-game-translator-translation" }, item2.translatedText), CE("div", {
    class: `bx-game-translator-original${this.settings.showOriginal ? "" : " bx-gone"}`
   }, item2.originalText));
   return $item.style.left = `${contentRect.left + item2.box.x * contentRect.width}px`, $item.style.top = `${contentRect.top + item2.box.y * contentRect.height}px`, $item.style.width = `${Math.max(64, item2.box.width * contentRect.width)}px`, $item.style.minHeight = `${height}px`, $item.style.fontSize = `${Math.max(12, Math.min(this.settings.fontSize, height * 0.72))}px`, $item.style.backgroundColor = `rgba(0, 0, 0, ${this.settings.backgroundOpacity / 100})`, $item;
  });
  this.$scene.replaceChildren(...elements);
 }
 clearSceneImmediately() {
  this.cancelSceneClear(), this.sceneItems = [], this.$scene.replaceChildren(), this.$scene.classList.add("bx-gone");
 }
 cancelSceneClear() {
  if (this.sceneClearTimerId !== null) window.clearTimeout(this.sceneClearTimerId), this.sceneClearTimerId = null;
 }
}
...
function readSettings() {
 return {
  ...
  mode: getGlobalPref("gameTranslator.mode"),
...
class GameTranslator {
 ...
 MAX_SCENE_TEXT_LINES = 8;
 MIN_SCENE_SCAN_INTERVAL = 750;
 MAX_SCENE_CHANGE_THRESHOLD = 0.005;
 ...
 start($video) {
  ...
  this.initializeTranslationContext(sessionId), this.disabledByError = !1, this.analyzePending = !1, this.frameCapture = new TranslatorFrameCapture(this.settings.ocrRegion, this.settings.mode), this.$video = $video, this.ocrEngine = new TesseractOcrEngine, this.stabilizer = new TextStabilizer(this.settings.stabilizationInterval, (text) => {
   if (sessionId === this.sessionId) this.onStableText(text, sessionId);
  }), this.overlay = new TranslationOverlay($video, this.settings), this.debugMetrics = { interval: this.getAnalysisInterval() }, this.overlay.updateDebug(this.debugMetrics), this.updateOverlayGeometry(), this.startTimer(sessionId), this.prepareProvider(), BxLogger.info(this.LOG_TAG, "Started", {
 ...
 getAnalysisInterval() {
  return this.settings.mode === "all-text" ? Math.max(this.settings.ocrInterval, this.MIN_SCENE_SCAN_INTERVAL) : this.settings.ocrInterval;
 }
 ...
 async analyze(sessionId) {
  ...
  if (this.settings.mode === "all-text") {
 ...
 async analyzeScene(detectionFrame, frameCapture, ocrEngine, sessionId) {
  ...
  if (!$ocrCanvas) return;
  this.ocrBusy = !0;
  let ocrStartedAt = performance.now();
  try {
   ...
   let translationStartedAt = performance.now(), translatedItems = await this.translateSceneLines(lines, abortController);
   if (sessionId !== this.sessionId || abortController.signal.aborted) return;
   if (this.debugMetrics.translationTime = performance.now() - translationStartedAt, this.overlay?.updateDebug(this.debugMetrics), translatedItems.length) this.overlay?.showScene(translatedItems);
   else this.overlay?.clearScene();
  } catch (error) {
   if (sessionId === this.sessionId) this.handleOcrError(error);
  } finally {
   this.finishOcr(sessionId);
 ...
 selectSceneTextLines(lines) {
  ...
  return lines.filter((line) => {
   ...
   return right.confidence + Math.min(25, right.box.width * right.box.height * 2000) - leftPriority;
  }).slice(0, this.MAX_SCENE_TEXT_LINES).sort((left, right) => left.box.y - right.box.y || left.box.x - right.box.x);
 }
 async translateSceneLines(lines, abortController) {
  let context = this.translationContext.snapshot(), translatedItems = [];
  for (let index = 0;index < lines.length; index += 2) {
   let batch = lines.slice(index, index + 2), translations = await Promise.all(batch.map(async (line) => {
    try {
     ...
     if (!abortController.signal.aborted) BxLogger.warning(this.LOG_TAG, "Scene text translation failed", line.text, error);
     return null;
    }
   }));
   if (translatedItems.push(...translations.filter((item2) => item2 !== null)), abortController.signal.aborted) break;
  }
 ...
 finishOcr(sessionId) {
  ...
  if (this.analyzePending && sessionId === this.sessionId) {
   if (this.analyzePending = !1, this.settings.mode === "subtitles") this.analyze(sessionId);
  }
 }
 ...
 onSettingsChanged(settingKey) {
  ...
  let providerChanged = previousSettings.provider !== this.settings.provider, providerConfigChanged = settingKey === "gameTranslator.deepl.proxyUrl", modeChanged = previousSettings.mode !== this.settings.mode;
  ...
  if (modeChanged) this.translationAbortController?.abort(), this.translationAbortController = null;
