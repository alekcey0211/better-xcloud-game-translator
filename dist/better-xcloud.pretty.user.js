...
class FrameChangeDetector {
 previousFrame = null;
 compare(frame, updateBaseline = !0) {
  let previousFrame = this.previousFrame;
  if (updateBaseline) this.previousFrame = new Uint8Array(frame);
  if (!previousFrame || previousFrame.length !== frame.length) return frame.some(Boolean) ? 1 : 0;
...
var MAX_RESCAN_INTERVAL = 1500;
class SubtitleScanGate {
 changeDetector = new FrameChangeDetector;
 lastScanAt = -1 / 0;
 evaluate(signature, threshold, now) {
  let changeScore = this.changeDetector.compare(signature, !1), shouldScan = changeScore >= threshold || now - this.lastScanAt >= MAX_RESCAN_INTERVAL;
  if (shouldScan) this.changeDetector.compare(signature), this.lastScanAt = now;
  return { changeScore, shouldScan };
 }
 reset() {
  this.changeDetector.reset(), this.lastScanAt = -1 / 0;
 }
}
...
function cleanRecognizedLine(text, confidence) {
 ...
 if (confidence < 35 || letters < 2 || !visibleCharacters || letters / visibleCharacters < 0.45) return "";
 return line;
}
...
function isLikelySubtitleText(rawText) {
 ...
 let latinLetters = text.match(/[a-z]/gi)?.length || 0, visibleCharacters = text.match(/\S/g)?.length || 0;
 return latinLetters / letters >= 0.7 && latinLetters / Math.max(1, visibleCharacters) >= 0.5 && !/(.)\1{3,}/i.test(normalized);
}
...
class GameTranslator {
 ...
 async analyze(sessionId) {
  ...
  if (!trackedLines.length) {
   this.scanGate.reset(), this.debugMetrics.changeScore = 0, this.overlay?.updateDebug(this.debugMetrics), this.stabilizer?.push("");
   return;
  }
  let { changeScore, shouldScan } = this.scanGate.evaluate(detection.signature, this.settings.changeThreshold, performance.now());
  if (this.debugMetrics.changeScore = changeScore, this.overlay?.updateDebug(this.debugMetrics), BxLogger.info(this.LOG_TAG, "Frame change", changeScore), !shouldScan) return;
  let $ocrCanvas;
  try {
